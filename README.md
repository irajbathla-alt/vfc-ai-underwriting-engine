...
- `apps-script/Bank_RBC.gs` — **RBC v4.3 candidate**, with printed-column and running-balance ledger parsing, exact four-control reconciliation and named-counterparty recurring-outflow detection.
...
Each RBC upload keeps its own statement ledger and locks the account number from the printed header when available. Before a statement contributes to a set, its saved rows are checked against that statement's dates, balances, credit/debit totals and, for current uploads, printed transaction counts. Re-uploading the same RBC account and statement period uses the latest verified current-rules ledger; a failed correction cannot displace a verified one. Other banks keep their prior frozen-ledger selection. Statements from distinct accounts and periods stay separate. Re-upload any RBC statements whose older spreadsheet rows were previously overwritten, because an overwritten ledger cannot be reconstructed from the spreadsheet alone.
...
Validate RBC v4.3 in the deployed Apps Script environment with statements from multiple companies, then lock its baseline. The local check covered 24 actual RBC PDFs from four account sets with both column-preserving and spacing-free row-preserving text, and rechecked their saved ledgers; the live PDF text service still needs validation. Train or approve the remaining bank candidates independently, one at a time.
