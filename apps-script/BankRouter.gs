...
function uploadStatementBatchByBank(bankId,companyName,files){
  ...
  try{
    ...
    // Uploading creates assessment inputs only. saveLenderDecision() is the sole promotion into historical training features.
    appendRows_('Uploads',uploadRows);appendRows_('PDF Summaries',pdfRows);appendRow_('Batch Summaries',[Utilities.getUuid(),companyName,period.label,files.length,period.earliest||'',period.latest||'',batch.combined_summary||'',batch.key_findings||'',batch.risks||'',batch.missing_info||'',new Date()]);rowsWritten=true;vfcFreezeDuplicateStatementFacts_(companyName,period.label);
...
function vfcFreezeDuplicateStatementFacts_(companyName,period){
  ...
  vfcGroupLogicalStatementRows_(rows).forEach(function(group){const bankId=vfcDetectBankId_(group[0]&&group[0].bank||'');
    // RBC re-uploads retain their individual verified ledgers. Selection is
    // read-only so a corrected statement can replace an earlier interpretation
    // without erasing the previous upload's audit trail.
    if(bankId==='RBC')return;
    const canonical=vfcCanonicalSignalRawFromRows_(group,bankId);if(!canonical)return;group.forEach(function(r){if(String(r.signalRaw||'')!==canonical)sh.getRange(r.rowNumber,signalCol+1).setValue(canonical);});});
}
...
function runBankingStackSelfTests(){
  ...
  function rbcFrozenRow(account,start,end,debit,createdAt,version){
    const current=version||vfcRbcBankProfile_().rulesVersion,opening=100,credit=100,closing=opening+credit-debit,tx=[{date:start,description:'Customer receipt',direction:'CREDIT',amount:credit},{date:start,description:'Loan payment NO.12345',direction:'DEBIT',amount:debit}],payload={version:6,transactionsVerified:true,bankId:'RBC',bankName:'RBC',accountNumber:account,statementStartDate:start,statementEndDate:end,openingBalance:opening,closingBalance:closing,totalDeposits:credit,totalWithdrawals:debit,bankRulesVersion:current,intakeContract:VFC_BANK_ENGINE.INTAKE_CONTRACT,transactions:tx};
    if(current===vfcRbcBankProfile_().rulesVersion)payload.statementLedgerAudit={verified:true,creditCount:1,debitCount:1,runtimeFingerprint:vfcRbcBankProfile_().runtimeFingerprint,source:'PRINTED_COLUMN_TRANSCRIPT'};
    return{companyName:'SYNTHETIC',period:'SET',bank:'RBC',fileName:'anonymous.pdf',startDate:start,endDate:end,createdAt:createdAt,signalRaw:VFC_BANK_ENGINE.CACHE_PREFIX+JSON.stringify(payload)};
  }
  ...
  test('Corrected RBC re-upload replaces the first verified interpretation of the same statement',function(){
    const old=rbcFrozenRow('ACCOUNT A','2026-01-01','2026-01-31',10,'2026-02-01','RBC-OLD'),first=rbcFrozenRow('ACCOUNT A','2026-01-01','2026-01-31',15,'2026-02-02'),corrected=rbcFrozenRow('ACCOUNT A','2026-01-01','2026-01-31',20,'2026-02-03'),groups=vfcGroupLogicalStatementRows_([old,first,corrected]);
    equal(groups.length,1,'one logical statement');
    const chosen=vfcParseBankCache_(vfcCanonicalSignalRawFromRows_(groups[0],'RBC'));
    equal(chosen.totalWithdrawals,20,'latest verified amount');equal(vfcParseBankCache_(first.signalRaw).totalWithdrawals,15,'old upload preserved');return'latest verified/current-rules statement';
  });
  test('Incomplete RBC correction is ignored and another account stays separate',function(){
    const good=rbcFrozenRow('ACCOUNT A','2026-01-01','2026-01-31',20,'2026-02-01'),bad=rbcFrozenRow('ACCOUNT A','2026-01-01','2026-01-31',30,'2026-02-02'),other=rbcFrozenRow('ACCOUNT B','2026-01-01','2026-01-31',45,'2026-02-03');
    const p=vfcParseBankCache_(bad.signalRaw);p.transactions.pop();bad.signalRaw=VFC_BANK_ENGINE.CACHE_PREFIX+JSON.stringify(p);
    const groups=vfcGroupLogicalStatementRows_([good,bad,other]);equal(groups.length,2,'different accounts');
    const chosen=vfcParseBankCache_(vfcCanonicalSignalRawFromRows_(groups.filter(function(g){return g.indexOf(good)>=0;})[0],'RBC'));
    equal(chosen.totalWithdrawals,20,'incomplete newer ledger rejected');return'bad upload cannot displace verified account';
  });
  test('A corrected statement contributes once to its own set of recurring obligations',function(){
    const older=rbcFrozenRow('ACCOUNT A','2026-01-01','2026-01-31',10,'2026-02-01'),corrected=rbcFrozenRow('ACCOUNT A','2026-01-01','2026-01-31',20,'2026-02-02'),next=rbcFrozenRow('ACCOUNT A','2026-02-01','2026-02-28',20,'2026-03-01'),groups=vfcGroupLogicalStatementRows_([older,corrected,next]),prepared=groups.map(function(g){const p=vfcParseBankCache_(vfcCanonicalSignalRawFromRows_(g,'RBC'));return{row:{fileName:'anonymous.pdf'},payload:vfcNormalizePayload_(p,g[g.length-1])};}),features=vfcBuildBankingFeatures_({},prepared);
    equal(groups.length,2,'two statement periods');equal(features.statementCount,2,'one ledger per period');
    close(features.existingMonthlyDebtService,20,.001,'recurring payment');equal(features.debtProfile.mostEvidentFinancingObligation.occurrences,2,'two observed months');return'two statements, one recurring obligation';
  });
