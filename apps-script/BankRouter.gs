...
function getBankParserTabs(){return vfcBankRegistry_().map(function(p){return{id:p.id,label:p.label,status:p.status,active:p.status==='LOCKED',rulesVersion:p.rulesVersion,intakeContract:p.intakeContract||VFC_BANK_ENGINE.INTAKE_CONTRACT};});}
function vfcGetBankProfile_(bankId){const id=String(bankId||'').toUpperCase().replace(/\s+/g,'_'),rows=vfcBankRegistry_();for(let i=0;i<rows.length;i++)if(rows[i].id===id)return rows[i];return{id:'UNKNOWN',label:'Unknown',status:'UNSUPPORTED',rulesVersion:'UNTRAINED',intakeContract:VFC_BANK_ENGINE.INTAKE_CONTRACT,aliases:[]};}
...
function vfcBankPreservePrintedDuplicate_(bankId,t,occurrence,items){switch(String(bankId||'').toUpperCase()){case'RBC':return vfcRbcPreservePrintedDuplicate_(t,occurrence,items);case'TD':return vfcTdPreservePrintedDuplicate_(t,occurrence,items);case'BMO':return vfcBmoPreservePrintedDuplicate_(t,occurrence,items);default:return false;}}
...
/** Banking Core owns the single frozen-ledger intake contract. */
...
function uploadStatementBatchByBank(bankId,companyName,files){
  ...
  }catch(e){if(!committed)vfcCleanupStagedFiles_(staged);throw e;}
}
...
/** Keep one stable canonical ledger per statement for the CURRENT rules generation. */
function vfcFreezeDuplicateStatementFacts_(companyName,period){
  ...
  Object.keys(groups).forEach(function(key){const rows=groups[key],bankId=vfcDetectBankId_(rows[0]&&rows[0].bank||''),canonical=vfcCanonicalSignalRawFromRows_(rows,bankId);if(!canonical)return;rows.forEach(function(r){if(String(r.signalRaw||'')!==canonical)sh.getRange(r.rowNumber,signalCol+1).setValue(canonical);});});
}
...
/** One command to validate the entire trained banking stack without Sheets/Drive/OpenAI. */
function runBankingStackSelfTests(){
  const router=[];function test(name,fn){try{router.push({name:name,pass:true,detail:String(fn()||'')});}catch(e){router.push({name:name,pass:false,detail:String(e&&e.message||e)});}}function equal(a,b,label){if(a!==b)throw new Error((label||'value')+' expected '+b+' but got '+a);}
