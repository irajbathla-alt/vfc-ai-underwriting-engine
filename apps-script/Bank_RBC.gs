...
function vfcRbcAuditFullLedger_(items,facts,text,fileName){
  ...
  if(printed.creditCount!==null&&creditCount!==printed.creditCount)throw new Error('RBC full-ledger credit count mismatch for '+fileName+': printed '+printed.creditCount+', extracted '+creditCount+'. Upload stopped.');
  if(printed.debitCount!==null&&debitCount!==printed.debitCount)throw new Error('RBC full-ledger debit count mismatch for '+fileName+': printed '+printed.debitCount+', extracted '+debitCount+'. Upload stopped.');
  const creditDiff=Math.abs(creditTotal-vfcNum_(facts.deposits)),debitDiff=Math.abs(debitTotal-vfcNum_(facts.withdrawals));
  if(creditDiff>.05||debitDiff>.05)throw new Error('RBC full-ledger totals do not match the printed Account Summary for '+fileName+'. CREDIT difference $'+vfcRound_(creditDiff,.01)+', DEBIT difference $'+vfcRound_(debitDiff,.01)+'. Upload stopped rather than guessing transaction direction or omitting rows.');
...
/** 25% multiplicative amount bands: materially different catch-up/manual amounts require their own recurrence. */
function vfcRbcAmountBand_(amount){
  const n=Math.max(1,vfcNum_(amount));
  return Math.round(Math.log(n)/Math.log(1.25));
}
...
function vfcRbcClassifyDebit_(t){
  ...
  if(/COMM\s+EQUIP\s+RENT\/LSE\s+SILVERCHEF|\bSILVERCHEF\b/.test(s)){
    family='FINANCING';entityKey='SILVERCHEF_EQUIPMENT_LEASE';label='SilverChef Equipment Lease';debtJustification='Explicit equipment lease wording plus recurring SilverChef payments.';
    ...
    family='FINANCING';entityKey='RBC_AFFIRM_CANADA';label='Affirm Canada';debtJustification='Affirm Canada is a financing counterparty; recurring observed payments are treated as financing debt.';
  }else if(/\bCANACAP\b/.test(s)){
    family='MCA';entityKey='CANACAP';label='Canacap';debtJustification='Known MCA-style business financing counterparty plus recurring observed payment cadence.';
  }else if(/\bGREENBOX\b/.test(s)){
    family='MCA';entityKey='GREENBOX';label='Greenbox';debtJustification='Known MCA-style business financing counterparty plus recurring observed payment cadence.';
  }else if(/\bICAPITAL\b/.test(s)){
    family='FINANCING';entityKey='ICAPITAL';label='iCapital';debtJustification='Known financing counterparty plus recurring observed payment cadence; iCapital is not assumed to be MCA without explicit MCA evidence.';
  }else if(/\bIPFS\b|PREMIUM\s+FINANC/.test(s)){
    family='FINANCING';entityKey='RBC_PREMIUM_FINANCE_'+vfcCounterpartyKey_(cp||raw)+'_B'+band;label=cp||'Premium Finance';debtJustification='Explicit premium-finance wording plus recurring observed payment cadence. Materially different one-time premium/catch-up amounts require independent recurrence.';
    ...
    const clean=raw.replace(/^AUTO\s+PAYMENT\s*/i,'').trim(),financeLike=/\bAFS\b|FINANC|LEASE|LENDING|DEALER\s+ADVANTAGE|AUTO\s+FINANCE|\bLOAN\b|\bMCA\b|\bMORTGAGE\b|\bLOC\b|LINE\s+OF\s+CREDIT|CREDIT\s+LINE|\bCANACAP\b|\bICAPITAL\b|\bGREENBOX\b|CAPITAL\s+(?:LENDING|FINANCE|FUNDING)|CREDIT\s+(?:CORP|FINANCE|LENDING)/.test(s);
    if(financeLike){family='FINANCING';entityKey='AUTO_PAYMENT_FINANCE_'+vfcCounterpartyKey_(clean||cp||raw);label=clean||cp||raw;debtJustification='Recurring automatic payment to a finance-like counterparty; AUTO PAYMENT alone is not sufficient.';}
    else{family='OTHER';entityKey='RBC_OTHER_AUTOPAY_'+vfcCounterpartyKey_(clean||cp||raw);label=clean||cp||raw;}
    ...
    const clean=raw.replace(/^MISC\s+PAYMENT\s*/i,'').trim();family='FINANCING';entityKey='AUTO_PAYMENT_FINANCE_'+vfcCounterpartyKey_(clean||cp||raw);label=clean||cp||raw;debtJustification='Recurring or retry payment to the same finance-like counterparty, even though RBC printed MISC PAYMENT instead of AUTO PAYMENT.';
  }else if(hasFinancingSignal){
    family='FINANCING';debtJustification='Explicit loan/mortgage/LOC/financing/lease/MCA wording plus recurring observed cadence.';
    if(/^LOAN\s+PAYMENT$/i.test(raw)){entityKey='GENERIC_LOAN_PAYMENT';label='Generic LOAN PAYMENT';}
    else{
      const numbered=s.match(/(?:NO\.?|NUMBER|#)\s*([0-9-]{5,})/),genericDebtNumber=s.match(/\b(?:LOAN|MORTGAGE|LOC)\b[^0-9]{0,30}([0-9][0-9-]{5,})\b/),ref=numbered||genericDebtNumber;
      if(ref){
...
/** Full-ledger count/sum verification makes exact duplicate Account Activity rows safe to preserve. */
function vfcRbcPreservePrintedDuplicate_(){return true;}
function vfcRbcStrongEntityKey_(key){
  return/^(RBC_AFFIRM_CANADA|RBC_PREMIUM_FINANCE_|RBC_BDC_|BDC|MERCHANT_GROWTH|JOURNEY_ONDECK|CANACAP|ICAPITAL|GREENBOX|SILVERCHEF_EQUIPMENT_LEASE|AUTO_PAYMENT_FINANCE_|RBC_FINANCE_|RBC_OTHER_|INSURANCE_|LEASE_[0-9]|FINANCE_REF_[0-9])/.test(String(key||'').toUpperCase());
}
...
function runRbcBankingSelfTests(){
  ...
  test('RBC full-ledger audit is fail-closed on count, sum or direction mismatch',function(){
    const text='Total deposits & credits (2) + 150.00\nTotal cheques & debits (2) - 30.00',facts={deposits:150,withdrawals:30},good=[tx('2026-01-01','Deposit A','CREDIT',100),tx('2026-01-02','Deposit B','CREDIT',50),tx('2026-01-03','Debit A','DEBIT',10),tx('2026-01-04','Debit B','DEBIT',20)];
    const a=vfcRbcAuditFullLedger_(good,facts,text,'test.pdf');close(a.totalCredits,150,.001,'credits');close(a.totalDebits,30,.001,'debits');
    let failed=false;try{vfcRbcAuditFullLedger_(good.map(function(x,i){return i===1?Object.assign({},x,{direction:'DEBIT'}):x;}),facts,text,'test.pdf');}catch(e){failed=true;}truthy(failed,'wrong direction must fail');
    return'full ledger reconciles or upload stops';
  });

  test('RBC AIM HIGH six printed deposit totals reproduce reported gross deposits',function(){
    const deposits=[45725.36,63708.46,56889.45,90909.53,70252.06,36387.79];close(vfcSum_(deposits),363872.65,.02,'total');close(vfcMean_(deposits),60645.4417,.02,'average');return'avg=60645.44';
  });

  test('RBC BR TO BR credit is internal transfer but customer e-Transfer is not',function(){
    truthy(vfcRbcIsNonOperatingTransferCredit_(tx('2026-01-14','BR TO BR - 0212','CREDIT',15000)),'BR TO BR transfer');equal(vfcRbcIsNonOperatingTransferCredit_(tx('2026-01-15','e-Transfer received CUSTOMER A','CREDIT',15000)),false,'customer e-transfer');return'transfers separated';
  });
  ...
  test('RBC BDC amount bands separate regular PAD, catch-up and manual loan payment',function(){
    ...
    equal(regular.entityKey,regular2.entityKey,'regular BDC band');if(regular.entityKey===catchup.entityKey)throw new Error('catch-up merged with regular PAD');if(regular.entityKey===manual.entityKey)throw new Error('manual merged with PAD');return regular.entityKey+' / '+catchup.entityKey+' / '+manual.entityKey;
  });

  test('RBC AIM HIGH returned BDC PADs are recognized and cannot inflate confirmed BDC debt',function(){
    const d=vfcDebtProfile_([
      row('2026-03-31',[tx('2026-03-27','Business PAD BDC','DEBIT',2578.34,'BDC')]),
      row('2026-04-30',[tx('2026-04-27','Business PAD BDC','DEBIT',2656.97,'BDC')]),
      row('2026-05-31',[tx('2026-05-27','Business PAD BDC','DEBIT',2606.59,'BDC'),tx('2026-05-27','Cheque returned NSF','CREDIT',2606.59,'Cheque returned NSF')]),
      row('2026-06-30',[tx('2026-06-29','Business PAD BDC','DEBIT',5289.91,'BDC'),tx('2026-06-29','Cheque returned NSF','CREDIT',5289.91,'Cheque returned NSF')]),
      row('2026-07-31',[tx('2026-07-27','Business PAD BDC','DEBIT',7952.39,'BDC'),tx('2026-07-28','Online Banking payment - 0071 BDC-LOAN/PRET','DEBIT',5324.91,'BDC')]),
      row('2026-08-31',[tx('2026-08-17','Business PAD BDC','DEBIT',2285.91,'BDC'),tx('2026-08-19','Loan BDC','CREDIT',5324.91,'BDC')])
    ]);
