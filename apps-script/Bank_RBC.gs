/**
 * RBC BANK ENGINE v3.0 — CANDIDATE / FINAL REVALIDATION
...
function vfcRbcBankProfile_(){
  return{
    ...
    label:'RBC',
    status:'CANDIDATE',
    rulesVersion:'RBC-3.0-CANDIDATE',
...
function vfcRbcLockFacts_(summary,text,fileName){
  ...
  if(statementDiff>.05)throw new Error('RBC printed Account Summary does not reconcile for '+name+'. Difference: $'+vfcRound_(statementDiff,.01)+'.');
  ...
  let ledger=vfcRbcPrepareLedger_(locked.banking_transactions||[]),audit=null,lastError='';
  for(let attempt=0;attempt<=2;attempt++){
    try{
      ...
      ledger=vfcRbcRepairLedger_(ledger,text,name,facts,lastError,attempt+1);
    }
  }

  locked.banking_transactions=ledger;
  locked.rbc_full_ledger_verified=true;
  locked.rbc_ledger_reconciled=!!(audit&&audit.reconciled);
