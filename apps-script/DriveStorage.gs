function getUploadFolder() {
  if (!CONFIG.DRIVE_UPLOAD_FOLDER_ID) throw new Error('Missing DRIVE_UPLOAD_FOLDER_ID in CONFIG');
...
function saveBase64FileToDrive(payload) {
  ...
  if (!rawData) throw new Error('Missing file data');
  const folder = getOrCreateApplicationFolder(payload);
...
function updateApplicationDocumentLinks(applicationId, fileUrl, folderUrl) {
  ...
  if (applicationIdColumn === -1 || documentLinksColumn === -1) throw new Error('Required Applications headers are missing');
...
function testDriveFolderConnection() {
  ...
  const result = { ok: true, folderId: folder.getId(), folderName: folder.getName() };
  Logger.log(JSON.stringify(result, null, 2));
  return result;
}

function testCreateApplicationFolder() {
  const result = createApplicationFolderForPayload({ applicationId: 'APP-TEST-123', businessName: 'ABC Pizza Ltd', ownerName: 'John Smith' });
  Logger.log(JSON.stringify(result, null, 2));
  return result;
}
