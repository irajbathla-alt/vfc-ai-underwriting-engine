...
/**
 * VFC Hybrid Underwriting Engine V2
 * Combines observed VFC lender outcomes, deterministic cash-flow analysis,
 * and an AI review. It never represents its conclusions as official lender criteria.
 */
function generatePowerAssessment(companyName, period) {
  ...
  rankings.forEach(function(result) {
    appendRow_('Hybrid Assessments', [
      assessmentId,
      VFC_POWER_CONFIG.MODEL_VERSION,
      companyName,
      period,
      result.lenderName,
      result.compositeScore,
      result.observedScore,
      fundamental.score,
      result.aiRiskScore,
      result.observedFit,
      result.confidence,
      result.historicalCases,
      result.similarCases,
      result.similarApprovals,
      result.similarDeclines,
      result.observedApprovalRate,
      result.lowApprovedAmount,
      result.highApprovedAmount,
      result.medianApprovedAmount,
      result.suggestedAmountLow,
      result.suggestedAmountHigh,
      result.reasoning,
      cleanCell_(result.conditions),
      cleanCell_(result.risks),
      new Date()
    ]);
  });
  ...
  appendRow_('Risk Scorecards', [
    ...
    fundamental.score,
    fundamental.grade,
    fundamental.cashFlowScore,
    fundamental.nsfScore,
    fundamental.balanceScore,
    fundamental.debtLoadScore,
    fundamental.coverageScore,
    fundamental.dataQualityScore,
    cleanCell_(fundamental.strengths),
    cleanCell_(fundamental.risks),
    cleanCell_(aiReview.missing_information),
    new Date()
  ]);
...
function buildPowerFeatures_(companyName, period) {
  ...
  const overdraftFlag = /overdraft|od fee|over limit|insufficient funds/.test(text) ? 1 : 0;
  const returnedPaymentFlag = /returned payment|returned item|reversal|chargeback/.test(text) ? 1 : 0;
  const suspectedStacking = /stack|multiple mca|multiple loan|several daily|several weekly/.test(text) ? 1 : 0;
  const missingInfoFlag = rows.some(function(r) { return String(r.missingInfo || '').trim().length > 3; }) ? 1 : 0;
  ...
  return Object.assign({}, base, {
    ...
    returnedPaymentFlag: returnedPaymentFlag,
    suspectedStacking: suspectedStacking,
...
function calculateFundamentalScore_(f) {
  ...
  return {
    ...
    debtLoadScore: debtLoadScore,
    coverageScore: coverageScore,
    dataQualityScore: dataQualityScore,
    strengths: unique_(strengths),
    risks: unique_(risks)
  };
}
...
function scorePowerLender_(lenderName, current, outcomes, fundamental, aiReview) {
  const records = outcomes.filter(function(row) { return sameText_(row.lenderName, lenderName); }).map(function(row) {
    ...
    return {
      ...
      features: features,
      ageWeight: ageWeight,
      similarity: features && features.statementCount ? powerSimilarity_(current, features) : 0
    };
  ...
  let composite = observedScore * VFC_POWER_CONFIG.HISTORICAL_WEIGHT +
    fundamental.score * VFC_POWER_CONFIG.FUNDAMENTAL_WEIGHT +
    aiRiskScore * VFC_POWER_CONFIG.AI_WEIGHT;
  ...
  const reasoning = similar.length
    ? approvals.length + ' of ' + similar.length + ' closest VFC cases were approved or conditional. Historical fit was blended with a ' + fundamental.grade.toLowerCase() + ' cash-flow risk grade.'
    : 'No comparable historical cases were available; the score relies primarily on general cash-flow analysis.';
  ...
  return {
    ...
    suggestedAmountLow: suggested.low,
    suggestedAmountHigh: suggested.high,
    reasoning: reasoning,
    ...
    similarCaseDetails: similar.slice(0,5).map(function(row) {
      return {
        companyName: row.companyName,
        period: row.period,
...
function createExpertReview_(features, fundamental) {
  const fallback = {
    risk_score: fundamental.score,
    risk_grade: fundamental.grade,
    executive_summary: 'General cash-flow analysis completed.',
...
function buildPowerDecision_(features, fundamental, aiReview, rankings) {
  ...
  const scoreGap = top.compositeScore && second.compositeScore ? top.compositeScore - second.compositeScore : 0;
  return {
